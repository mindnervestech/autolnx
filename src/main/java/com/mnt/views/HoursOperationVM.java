package com.mnt.views;

import java.sql.Date;

public class HoursOperationVM {
	public String friCloseTime;
	public String friOpenTime;
	public String monOpenTime;
	public String monCloseTime;
	public String tueCloseTime;
	public String getFriCloseTime() {
		return friCloseTime;
	}
	public void setFriCloseTime(String friCloseTime) {
		this.friCloseTime = friCloseTime;
	}
	public String getFriOpenTime() {
		return friOpenTime;
	}
	public void setFriOpenTime(String friOpenTime) {
		this.friOpenTime = friOpenTime;
	}
	public String getMonOpenTime() {
		return monOpenTime;
	}
	public void setMonOpenTime(String monOpenTime) {
		this.monOpenTime = monOpenTime;
	}
	public String getMonCloseTime() {
		return monCloseTime;
	}
	public void setMonCloseTime(String monCloseTime) {
		this.monCloseTime = monCloseTime;
	}
	public String getTueCloseTime() {
		return tueCloseTime;
	}
	public void setTueCloseTime(String tueCloseTime) {
		this.tueCloseTime = tueCloseTime;
	}
	public String getTueOpenTime() {
		return tueOpenTime;
	}
	public void setTueOpenTime(String tueOpenTime) {
		this.tueOpenTime = tueOpenTime;
	}
	public String getWedCloseTime() {
		return wedCloseTime;
	}
	public void setWedCloseTime(String wedCloseTime) {
		this.wedCloseTime = wedCloseTime;
	}
	public String getWedOpenTime() {
		return wedOpenTime;
	}
	public void setWedOpenTime(String wedOpenTime) {
		this.wedOpenTime = wedOpenTime;
	}
	public String getThuCloseTime() {
		return thuCloseTime;
	}
	public void setThuCloseTime(String thuCloseTime) {
		this.thuCloseTime = thuCloseTime;
	}
	public String getThuOpenTime() {
		return thuOpenTime;
	}
	public void setThuOpenTime(String thuOpenTime) {
		this.thuOpenTime = thuOpenTime;
	}
	public String getSatCloseTime() {
		return satCloseTime;
	}
	public void setSatCloseTime(String satCloseTime) {
		this.satCloseTime = satCloseTime;
	}
	public String getSatOpenTime() {
		return satOpenTime;
	}
	public void setSatOpenTime(String satOpenTime) {
		this.satOpenTime = satOpenTime;
	}
	public String getSunCloseTime() {
		return sunCloseTime;
	}
	public void setSunCloseTime(String sunCloseTime) {
		this.sunCloseTime = sunCloseTime;
	}
	public String getTypeOfOperation() {
		return typeOfOperation;
	}
	public void setTypeOfOperation(String typeOfOperation) {
		this.typeOfOperation = typeOfOperation;
	}
	public String getSunOpenTime() {
		return sunOpenTime;
	}
	public void setSunOpenTime(String sunOpenTime) {
		this.sunOpenTime = sunOpenTime;
	}
	public Boolean getFriOpen() {
		return friOpen;
	}
	public void setFriOpen(Boolean friOpen) {
		this.friOpen = friOpen;
	}
	public Boolean getSunOpen() {
		return sunOpen;
	}
	public void setSunOpen(Boolean sunOpen) {
		this.sunOpen = sunOpen;
	}
	public Boolean getMonOpen() {
		return monOpen;
	}
	public void setMonOpen(Boolean monOpen) {
		this.monOpen = monOpen;
	}
	public Boolean getTueOpen() {
		return tueOpen;
	}
	public void setTueOpen(Boolean tueOpen) {
		this.tueOpen = tueOpen;
	}
	public Boolean getWedOpen() {
		return wedOpen;
	}
	public void setWedOpen(Boolean wedOpen) {
		this.wedOpen = wedOpen;
	}
	public Boolean getThuOpen() {
		return thuOpen;
	}
	public void setThuOpen(Boolean thuOpen) {
		this.thuOpen = thuOpen;
	}
	public Boolean getSatOpen() {
		return satOpen;
	}
	public void setSatOpen(Boolean satOpen) {
		this.satOpen = satOpen;
	}
	public int getSunFlag() {
		return sunFlag;
	}
	public void setSunFlag(int sunFlag) {
		this.sunFlag = sunFlag;
	}
	public int getMonFlag() {
		return monFlag;
	}
	public void setMonFlag(int monFlag) {
		this.monFlag = monFlag;
	}
	public int getTueFlag() {
		return tueFlag;
	}
	public void setTueFlag(int tueFlag) {
		this.tueFlag = tueFlag;
	}
	public int getWedFlag() {
		return wedFlag;
	}
	public void setWedFlag(int wedFlag) {
		this.wedFlag = wedFlag;
	}
	public int getThuFlag() {
		return thuFlag;
	}
	public void setThuFlag(int thuFlag) {
		this.thuFlag = thuFlag;
	}
	public int getFriFlag() {
		return friFlag;
	}
	public void setFriFlag(int friFlag) {
		this.friFlag = friFlag;
	}
	public int getSatFlag() {
		return satFlag;
	}
	public void setSatFlag(int satFlag) {
		this.satFlag = satFlag;
	}
	public String tueOpenTime;
	public String wedCloseTime;
	public String wedOpenTime;
	public String thuCloseTime;
	public String thuOpenTime;
	public String satCloseTime;
	public String satOpenTime;
	public String sunCloseTime;
	public String typeOfOperation;
	public String sunOpenTime;
	public Boolean friOpen;
	public Boolean sunOpen;
	public Boolean monOpen;
	public Boolean tueOpen;
	public Boolean wedOpen;
	public Boolean thuOpen;
	public Boolean satOpen;
	public int sunFlag;
	public int monFlag;
	public int tueFlag;
	public int wedFlag;
	public int thuFlag;
	public int friFlag;
	public int satFlag;
	
	
}
